...
local function findClasses(categoryFile)
   local classList = {} 
   local classToIdx = {}
   for line in io.lines(categoryFile) do
      local class = string.split(line, " ")[1]
      class = string.sub(class, 2)
      local idx = string.split(line, " ")[2]+1
   ...
   -- assert(#classList == 1000, 'expected 401 Places2 classes')
   return classList, classToIdx
end

local function findImages(dir, classToIdx, opt, dir_type)
   ...
   while true do
      ...
      local s,e = string.find(dirName, dir)
      className = string.sub(dirName, e+2)
      ...
      if opt.subdata ~= 0 then
         if dir_type == 'train' then
            if tonumber(filename:sub(0,8))<=opt.subdata then
...
function M.exec(opt, cacheFile)
   ...
   local classFileDir = paths.concat(opt.data, 'categories_places365.txt')
   ...
   local classList, classToIdx = findClasses(classFileDir)

   print(" | finding all validation images")
   local valImagePath, valImageClass = findImages(valDir, classToIdx, opt, 'val')

   print(" | finding all training images")
   local trainImagePath, trainImageClass = findImages(trainDir, classToIdx, opt, 'train')
